...
$files = get-childitem -path "C:\dev.cm\cm-cowmanager\Source\2. Services\CowManager.Api\bin" -recurse -filter "*.dll"
$files += get-childitem -path "C:\dev.cm\cm-cowmanager\Source\2. Services\CowManager.Api\bin" -recurse -filter "*.exe"
 
$binaries = $files | 
    ForEach-Object { 
        New-Object psobject -Property @{ 
            Path = $_ 
            ExeType = Get-ExecutableType -Path $_.FullName
        } 
    } 

$binaries | sort-object | Format-Table -AutoSize 