...
select c.SourceLastSyncCowData, 
	max(c.DateLastSyncCowData) as DateLastSync, 
	GETUTCDATE() as DateNowUTC, 
	min(datediff(minute, c.DateLastSyncCowData, getutcdate())) as LastSyncInMinutes
from koe.bedrijf b
...
foreach ($result in $resultset)
{
    ...
    $pPrtgObject = New-PrtgObject `
            ...
            -value ($result.LastSyncInMinutes) `
            -unit "min" `
            -mode "Absolute" `
            -showChart 1 `
            -LimitMaxError 0 `
